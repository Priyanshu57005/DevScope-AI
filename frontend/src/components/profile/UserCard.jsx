...
const UserCard = ({ user, cached }) => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto mt-10 animate-fade-in relative">
      {cached && (
        <div 
          className="absolute -top-3 right-4 z-10 px-2.5 py-1 text-[10px] uppercase font-mono font-bold tracking-wider rounded-full border border-teal-500/30 bg-teal-950/40 text-teal-400 backdrop-blur-md"
        >
          ⚡ Served from Cache
        </div>
      )}
      <div className="relative bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl overflow-hidden hover:border-[var(--border-glow)] transition-all duration-500 hover:shadow-[0_0_40px_#388bfd15]">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid grid-cols-3 gap-3 mt-8">
            <StatBadge label="Repositories" value={user.publicRepos} icon="📦" />
